...
async function createMatch(seedMatch: SeedMatch) {
...
async function insertCommentary(matchId: number, entry: CommentaryEntry) {
...
function inningsRank(period: string | null | undefined) {
  if (!period) {
    return 0;
  }
...
function cricketBattingTeam(entry: CommentaryEntry, match: Match) {
...
function normalizeCricketFeed(entries: CommentaryEntry[], match: Match) {
  ...
  for (const key of inningsKeys) {
    ...
    const primaryTeam = inningsEntries.find(
      (entry: CommentaryEntry) =>
        entry.team === match.homeTeam || entry.team === match.awayTeam
    )?.team;
    const secondaryTeam =
      primaryTeam === match.homeTeam ? match.awayTeam : match.homeTeam;

    const neutral = inningsEntries.filter(
      (entry: CommentaryEntry) => !entry.team || entry.team === 'neutral'
    );
    const primary = inningsEntries.filter(
      (entry: CommentaryEntry) => entry.team === primaryTeam
    );
    const secondary = inningsEntries.filter(
      (entry: CommentaryEntry) => entry.team === secondaryTeam
    );
...
function cloneCommentaryEntries(
  ...
  return entries.map((entry: CommentaryEntry) => {
    ...
    next.message = replaceTrailingTeam(entry.message, replacements) as
      | string
      | undefined;
    return next;
  });
